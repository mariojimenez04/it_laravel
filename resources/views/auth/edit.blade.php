...
@section('titulo')
    Administracion - Actualizar usuario - {{ $usuario->name }}
@endsection

@section('contenido')
<form id="formulario" class="container" action="{{ route('users.update', $usuario) }}" method="POST">
    @csrf
    <div class="row gap-5">
        ...
        <div class="col-sm-5">
            ...
            <input type="email" id="email" name="email" class="form-control form-control-sm @error('email') is-invalid @enderror" value="{{ $usuario->email }}" disabled>
        ...
        <div class="col-md-12">
            ...
            <input type="text" id="nombre" name="nombre" class="form-control form-control-sm @error('nombre') is-invalid @enderror" value="{{ $usuario->name }}">
